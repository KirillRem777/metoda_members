...
function members_enqueue_scripts() {
    ...
    if (is_post_type_archive('members') || is_singular('members')) {
        ...
        wp_localize_script('members-archive-ajax', 'membersAjax', array(
            ...
            'nonce' => wp_create_nonce('public_members_nonce')  // FIX: совпадает с check_ajax_referer в filter_members_ajax
        ));
    }
 ...
 *
 * DEPRECATED v4.0.2: Этот handler перемещён в class-member-dashboard.php
 * с поддержкой admin bypass для редактирования чужих профилей.
 *
