...
if (!current_user_can('manage_options')) {
    die('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Доступ запрещён</title><style>body{font-family:sans-serif;padding:50px;text-align:center;background:#f5f5f5;}h1{color:#dc3545;}</style></head><body><h1>❌ Доступ запрещён</h1><p>У вас нет прав для выполнения этой операции.</p><p>Войдите как администратор: <a href="' . wp_login_url() . '">Войти</a></p></body></html>');
}

// ⚠️ ВАЖНО: Это одноразовый скрипт - УДАЛИ его после использования!
...
// Маппинг английских названий файлов на русские ФИО
$english_to_russian = array(
    'berdnikova' => 'Бердникова Ирина Евгеньевна',
    'fahretdinova' => 'Фахретдинова Айсылу Амировна',
    'golubitskaya' => 'Голубицкая Татьяна Анатольевна',
    'ilyina' => 'Ильина Людмила Николаевна',
...
function extract_member_name($filename, $mapping = array()) {
    $name = basename($filename);

    // Удаляем расширение
    $name = preg_replace('/\.(jpg|jpeg|png|gif|webp)$/i', '', $name);

    // Удаляем цифры в конце (berdnikova1, berdnikova2)
    $name = preg_replace('/\d+$/', '', $name);

    // Удаляем -2, -3 и т.д. в конце (дубликаты фото)
    $name = preg_replace('/-\d+$/', '', $name);

    // Проверяем маппинг для английских имен
...
foreach ($files as $file) {
    $member_name = extract_member_name($file, $english_to_russian);
