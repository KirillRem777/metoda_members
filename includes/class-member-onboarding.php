...
class Member_Onboarding {
    ...
    private function ajax_complete_access_code_onboarding() {
        ...
        // Telegram integration (если пользователь хочет подключить)
        $telegram_link = null;
        $enable_telegram = isset($_POST['enable_telegram']) && $_POST['enable_telegram'] === 'true';

        if ($enable_telegram && class_exists('Metoda\\Auth\\Telegram')) {
            $telegram = \Metoda\Auth\Telegram::instance();
            if ($telegram->is_configured()) {
                $telegram_link = $telegram->get_link_url($user_id);
            }
        }
